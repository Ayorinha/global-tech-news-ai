...
async function loadProjects(){if(!$sbList)return;try{const r=await fetch("data/projects.json?v="+Date.now(),{cache:"no-store"});if(!r.ok)throw new Error("HTTP "+r.status);const data=await r.json();if(!Array.isArray(data))throw new Error("Formato inválido de projects.json");state.projects=data;}catch(e){console.error("Erro projects:",e);state.projects=[]}renderPortfolio()}
