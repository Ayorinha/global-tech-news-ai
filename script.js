...
function getFiltered(){
  ...
  var list=state.articles.filter(function(a){
    ...
    if(!q) return true;
    var h=[get(a,"title_pt","title_original","title"),
           get(a,"description_pt","description_original"),
           get(a,"source")].join(" ").toLowerCase();
...
function getWeekTop(all){
  ...
  return all
    .filter(function(a){return new Date(a.published||0).getTime()>cut;})
    .sort(function(a,b){
...
function weekCard(a,idx){
  ...
  return '<a class="week-card" href="'+link+'" target="_blank" rel="noopener" '
    ...
    +'<div class="week-meta"><span>'+time+'</span>'
    +(views>0?'<span class="week-views">👁 '+views+' acessos</span>':"")
    +'</div></div></a>';
}
...
function newsItem(a,idx){
  ...
  var thumb="";
  if(image){
    thumb='<div class="news-thumb">'+makeImg(image,title,emoji,"news-thumb-ph")+'</div>';
  }
...
function renderNews(){
  ...
  $week.innerHTML=top.length?top.map(weekCard).join(""):'<p class="empty">Sem destaques esta semana.</p>';
...
function buildFilters(){
  ...
  Object.keys(LANGS).forEach(function(code){
    var info=LANGS[code],c=counts[code]||0;
    if(!c) return;
...
function renderPortfolio(){
  ...
  var items=state.projects.filter(function(p){
    return tab==="projetos"?p.category==="projeto":p.category==="estudo";
  });

  if(!items.length){
    $sbList.innerHTML='<div class="add-hint">Nenhum item ainda.<br>'
      +'Edite <a href="https://github.com/Ayorinha/global-tech-news-ai/edit/main/data/projects.json" '
      +'target="_blank">data/projects.json</a></div>';
    return;
  }
  ...
  var html=items.map(function(p){
    ...
    var lnkHtml="";
    if(p.link) lnkHtml+='<a class="proj-link" href="'+esc(p.link)+'" target="_blank" rel="noopener">'
      +'<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">'
      +'<path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/>'
      +'<polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>Ver site</a>';
    if(p.repo) lnkHtml+='<a class="proj-link" href="'+esc(p.repo)+'" target="_blank" rel="noopener">'
      ...
      +' 0 .315.225.69.825.57A12.02 12.02 0 0 0 24 12c0-6.63-5.37-12-12-12z"/></svg>GitHub</a>';

    var metaHtml="";
    if(p.created_at) metaHtml+='<span class="proj-tag" style="color:#00d4ff;border-color:rgba(0,212,255,.3)">📅 '+esc(p.created_at)+'</span>';
    if(p.version)    metaHtml+='<span class="proj-tag" style="color:#00e676;border-color:rgba(0,230,118,.3)">v'+esc(p.version)+'</span>';
    ...
    return '<div class="proj-card">'
      ...
      +(metaHtml?'<div class="proj-tags" style="margin-bottom:.4rem">'+metaHtml+'</div>':"")
      +(p.tags&&p.tags.length?'<div class="proj-tags">'+p.tags.map(function(t){return '<span class="proj-tag">'+esc(t)+'</span>';}).join("")+'</div>':"")
      +(lnkHtml?'<div class="proj-links">'+lnkHtml+'</div>':"")
      +'</div>';
  }).join("");

  html+='<div class="add-hint">Adicione projetos em '
...
async function loadNews(){
  ...
  try{
    ...
    $list.innerHTML='<div class="empty">Erro ao carregar: '+esc(err.message)+'</div>';
    $week.innerHTML="";
  }
}
