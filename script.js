...
const LANGS={pt:{label:"Português Brasil",flag:"🇧🇷"},es:{label:"Español",flag:"🇪🇸"},en:{label:"English",flag:"🇬🇧"}};
...
const currentLang=()=>window.AyoraiI18n?.lang?.()||localStorage.getItem("ayorai_lang")||"pt";
...
async function loadProjects(){if(!$sbList)return;try{const r=await fetch("data/projects.json?v="+Date.now(),{cache:"no-store"});state.projects=r.ok?await r.json():[]}catch(e){state.projects=[]}renderPortfolio()}
...
document.addEventListener("DOMContentLoaded",()=>{if(window.innerWidth<=768&&$layout)$layout.classList.add("sidebar-closed");loadNews();loadProjects()});
